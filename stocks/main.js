...
async function refreshData() {
    showLoading(true);
    try {
        const [itemsCsv, inventoryCsv] = await Promise.all([
            fetch(ITEMS_URL).then(res => res.text()),
            fetch(INVENTORY_URL).then(res => res.text())
        ]);
...
function renderInventory(filter = '') {
    ...
    grid.innerHTML = filteredItems.map(item => {
        ...
        // Priority 1: Local mapping
        // Priority 2: AppSheet URL
        // Priority 3: Base64 Placeholder
        let imageUrl = BASE64_PLACEHOLDER;
        const localImg = imageMapping[item['Item ID']];
...
function openEditModal(itemId) {
    ...
    const imageMapping = {
        'Hdd.': 'img/hdd ssd.png',
        'Hdd SSD.': 'img/hdd ssd.png',
        'Hdd M.2': 'img/hdd m2.png',
        'RAM': 'img/ram.png',
        'RAM NB': 'img/ram nb.png',
        'PSU': 'img/psu.png',
        'KB+Mo': 'img/kb.png',
        'Keyboard': 'img/kb.png',
        'Mouse': 'img/mouse.png',
        'Mouse chi': 'img/mouse.png',
        'Batt': 'img/batt.png',
        'Switch': 'img/switch.png',
        'Wifi': 'img/wifi.png'
    };
    
    modalImg.src = imageMapping[item['Name']] || (item['Image'] ? `https://www.appsheet.com/template/gettablefileurl?appName=Inventory-43814408&tableName=Items&fileName=${encodeURIComponent(item['Image'])}` : BASE64_PLACEHOLDER);
